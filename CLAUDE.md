...
- **Null narrowing**: prefer Dart 3 case-patterns over the `!` operator. Use `if (x case final v?) { ... v ... }` instead of `if (x != null) { ... x! ... }`, and combine with `when` for extra guards (`if (s.state case final state? when state.isNotEmpty)`). The case-pattern introduces a non-null local without runtime assertion.
- **Capturing log output in tests**: use `mocktail`'s `Mock implements LDLogAdapter` plus `verify(() => adapter.log(captureAny())).captured` rather than hand-rolling a capturing adapter. See `packages/common_client/test/persistence/validating_persistence_test.dart` for the canonical setup (including the `registerFallbackValue(LDLogRecord(...))` in `setUpAll`).
