...
class _DemoPageState extends State<DemoPage> {
  ...
  // null means no override is set: the SDK resolves the mode automatically.
  // The app starts in automatic mode; selecting a specific mode sets a
  // sticky override.
  ConnectionModeId? _selectedMode;
  ...
  void initState() {
    ...
    _client = LDClient(
        LDConfig(
          ...
          // Automatic state detection is left at the platform defaults
          // (enabled on mobile). In automatic mode the SDK follows the
          // application lifecycle -- for example switching to the background
          // connection mode when the app is backgrounded. Selecting a
          // specific mode below applies a sticky override that suppresses
          // these automatic transitions until cleared by choosing Automatic.
        ),
  ...
  void _setMode(ConnectionModeId? mode) {
    setState(() {
      _selectedMode = mode;
    });
    // A null mode clears the override and resumes automatic resolution.
            ...
            : SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Laid out vertically so the full label for each mode is
                    // readable on a phone-width screen. "Automatic" clears the
                    // override and resumes automatic mode resolution.
                    for (final entry in const <(String, ConnectionModeId?)>[
                      ('Automatic', null),
                      ...
                      ListTile(
                        title: Text(entry.$1),
                        leading: Icon(_selectedMode == entry.$2
                            ? Icons.radio_button_checked
                            : Icons.radio_button_unchecked),
                        selected: _selectedMode == entry.$2,
                        onTap: () => _setMode(entry.$2),
